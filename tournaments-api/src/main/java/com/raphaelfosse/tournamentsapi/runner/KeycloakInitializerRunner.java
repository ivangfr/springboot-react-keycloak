package com.raphaelfosse.tournamentsapi.runner;

import com.raphaelfosse.tournamentsapi.security.WebSecurityConfig;
...
public class KeycloakInitializerRunner implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        clientRepresentation.setClientId(TOURNAMENTS_APP_CLIENT_ID);
        clientRepresentation.setDirectAccessGrantsEnabled(true);
        clientRepresentation.setPublicClient(true);
        clientRepresentation.setRedirectUris(List.of(TOURNAMENTS_APP_REDIRECT_URL));
        clientRepresentation.setDefaultRoles(new String[]{WebSecurityConfig.TOURNAMENTS_USER});
        realmRepresentation.setClients(List.of(clientRepresentation));

        // Users
        List<UserRepresentation> userRepresentations = TOURNAMENTS_USER_LIST.stream()
                .map(userPass -> {
                    // User Credentials
        ...
        // Testing
        UserPass admin = TOURNAMENTS_USER_LIST.get(0);
        log.info("Testing getting token for '{}' ...", admin.username());

        Keycloak keycloakTournamentApp = KeycloakBuilder.builder().serverUrl(KEYCLOAK_SERVER_URL)
                .realm(COMPANY_SERVICE_REALM_NAME).username(admin.username()).password(admin.password())
                .clientId(TOURNAMENTS_APP_CLIENT_ID).build();

        log.info("'{}' token: {}", admin.username(), keycloakTournamentApp.tokenManager().grantToken().getToken());
    ...
    private Map<String, List<String>> getClientRoles(UserPass userPass) {
        ...
        roles.add(WebSecurityConfig.TOURNAMENTS_USER);
        if ("admin".equals(userPass.username())) {
            roles.add(WebSecurityConfig.TOURNAMENTS_MANAGER);
        }
        return Map.of(TOURNAMENTS_APP_CLIENT_ID, roles);
    }
    ...
    private static final String TOURNAMENTS_APP_CLIENT_ID = "tournaments-app";
    private static final String TOURNAMENTS_APP_REDIRECT_URL = "http://localhost:3000/*";
    private static final List<UserPass> TOURNAMENTS_USER_LIST = Arrays.asList(
